...
public class Test01 {
	...
	public static List<Object> exists(ArrayList<Product> arrlist, int num, boolean flag, int index) {
	...
	public static void main(String[] args) {
		...
		boolean flagEditCnt = false; // 상품재고 변경 flag
				...
				while (true) {
					...
					if (action == 1) { // 1번 입력시 상품추가
						...
						while (flagEditCnt) {						
							...
							System.out.println("\n" + data.get(pIndex).getName() + "을(를) " + cnt + "개 " + (flagAddSub ? "증가" : "감소") + "시키시겠습니까?");
							while (true) {
								printYesOrNoMsg();
								printInputMsg();
								action = sc.nextInt();
