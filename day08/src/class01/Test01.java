	...
	public static void main(String[] args) {
		...
		while (true) {
			...
			if (action == 5) {
				...
				System.out.print("입력) ");
				int num = sc.nextInt();
				if (num <= 0 || index < num) { // 혹시 num이 유효하지않아?
					// ☆ 경계값 검사
			...
			} else { // 유효성 검사(사용자의 입력값 검사) >> 위해 else 만든다
				System.out.println("잘못된 번호입니다! 확인 후 다시 이용해주세요");
			}
